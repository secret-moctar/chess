...
class ChessEng(ChessAbc):
    ...
    def get_fnn(self):
        ...
        return self.board.get_fnn(
            self.curr_player, w_k, b_k, self.last_move, self.half_clock, self.full_clock
        )

    def load_fnn(self, squence):
        turn, c_w, c_b, last_move, hc, fc = self.board.load_fnn(
            squence, debug=True)
        ...
        self.half_clock = hc
        self.full_clock = fc
        self.update_mvt(real=False)

    def copy(self):
        other = ChessEng(self.board.rel_pos, self.w_clock, self.b_clock, org=False)
    ...
    def is_move_valid(self, move):
        ...
        if (
            piece
            and piece.fen.lower() == "p"
            and move.start % 8 != move.end % 8
            and not temp.board.get_piece(move.end)
        ):
            # Ensure it's a valid en passant move
            if temp.last_move and abs(temp.last_move.start - temp.last_move.end) == 16:
                captured_pawn_pos = (
                    move.end + 8 if piece.get_type() == WH else move.end - 8
                )
        ...
        if curr and curr.fen.lower() == "k" and real:
            if curr.castling[0]:
                ...
                for s in range(1, 4):
                    ...
                    if self.board.get_piece(ct_s):
                        do_it = False
                if do_it:
                    if self.is_move_valid(Move(me, me - 2, False)):
                        hall.append(Move(me, me - 2, False))
            if curr.castling[1]:
                do_it = True
                for s in range(1, 3):
                    ct_s = me + s  # castling target square
    ...
    def handle_en_passant(self, me, curr, hall):
        if (
            curr.fen.lower() == "p"
            and self.last_move
            and abs(self.last_move.start - self.last_move.end) == 16
        ):  # Two-square pawn move
            if curr.get_type() == WH and me // 8 == 3:  # White pawn on 5th rank
                if me % 8 != 0 and self.last_move.end == me - 1:  # Left side
                    if self.is_move_valid(Move(me, me - 9, True)):
                        hall.append(Move(me, me - 9, True))
                if me % 8 != 7 and self.last_move.end == me + 1:  # Right side
                    if self.is_move_valid(Move(me, me - 7, True)):
                        hall.append(Move(me, me - 7, True))
    ...
    def get_all_legal_moves(self, me, moves, real=True):
        ...
        for m in moves:
            ...
            if curr.fen.lower() == "p":
                if m % 8 == me % 8:
                    ...
                    if abs(m // 8 - me // 8) == 2:
                        ...
                        if self.board.get_piece(m + sign * 8):
                            continue
                else:
                    if not peice:
                        continue
            ...
            if peice and peice.is_friend(curr):
                continue
            if real:
                if self.is_move_valid(Move(me, m, self.is_cap(m))):
    ...
    def pawn_special_move(self, move, piece, real=True):
        if (
            ...
            and piece.fen.lower() == "p"
            and move.start % 8 != move.end % 8
            and not self.board.get_piece(move.end)
        ):
    ...
    def pawn_promotion(self, move, piece, real=True):
        if piece and piece.fen.lower() == "p" and move.end // 8 in {0, 7}:
            if real:
                res_manager.get_resource("promote").play()
            self.board.in_board[move.end] = Queen(
                "Q" if piece.get_type() == WH else "q"
            )
    ...
    def make_move(self, move, real=True):
        ...
        if (
            ...
            and piece
            and piece.fen.lower() == "p"
            and abs(move.end - move.start) == 16
        ):
            self.last_move = move
        else:
            self.last_move = None
        ...
        if self.is_king_checked(self.curr_player):
            if real:
                res_manager.get_resource("check").play()
        ...
        if real:
            self.history.append(self.get_fnn())
        if real:
            if self.last_move:
                print(f"last: {self.last_move.start} -> {self.last_move.end} ")
            else:
                print("last: None")
    ...
    def update_mvt(self, real=True, debug=False):
        for i in range(64):
            ...
            piece.all_legal_moves = self.get_all_legal_moves(
                i, piece.all_moves, real=real
            )
        if debug:
        ...
        for square in threat:
            if threat[square]:
                pg.draw.rect(
                    screen,
                    (255, 0, 0),
                    ((square % 8) * SQUA, (square // 8) * SQUA, SQUA, SQUA),
                )
    ...
    def render(self, screen):
        ...
        if self.selected > -1:
            ...
            self.board.draw_selected(screen, self.selected)
